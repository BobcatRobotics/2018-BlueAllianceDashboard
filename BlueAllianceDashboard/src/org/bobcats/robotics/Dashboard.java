...
import org.bobcats.robotics.json.District;
import org.bobcats.robotics.json.SimpleEventList;
	...
	private static void generateXLSFiles() {
		...
		try {
			...
			for (String teamKey : teams) {
				XLSTeamEventList xlsReport2 = new XLSTeamEventList(teamKey);
				xlsReport2.createReport(xlsReport2.getReportName());
				List<String> eventKeys = xlsReport2.getData();
	...
	private static void generateTextFiles() {
		try {
			...
			for (GameData match : matchResults) {
				...
				Scores scOpp = match.getOpponentsTeamScores();
				avg.addScores(sc);
				ReportGenerator.log(String.format(matchStr, match.getShortName(), sc.getAutoScaleOwnershipSec(),
						...
						scOpp.getTeleopSwitchOwnershipSec(), sc.getVaultPoints(), sc.endGameStatus(teamPosition),
						sc.getAutoQuestRankingPoint(), sc.getFaceTheBossRankingPoint(), match.getMatchStatus()));
			}
			ReportGenerator.log(" ");
